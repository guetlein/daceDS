...
InteractionHandler::InteractionHandler(std::shared_ptr<Interaction> i, std::shared_ptr<Producer> p) : MessageHandler() {
    api = i;
    producer = p;
}
...
void InteractionHandler::parseScopeVehicle(ConsumedMessage* msg, InteractionTopicMetadata2& imeta) {
    std::string vehicleID = msg->key;
...
void InteractionHandler::parseScopeWorld(ConsumedMessage* msg, InteractionTopicMetadata2& imeta) {
    std::string vehicleID = msg->key;
